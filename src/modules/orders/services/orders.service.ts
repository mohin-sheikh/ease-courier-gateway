...
export class OrdersService {
  ...
  async create(dto: CreateOrderDto): Promise<OrderEntity> {
    if (dto.idempotencyKey) {
      const existingByKey = await this.orderRepository.findByIdempotencyKey(
        dto.idempotencyKey,
      );
    ...
    const existing = await this.orderRepository.findByInternalOrderId(
      dto.internalOrderId,
      dto.courierPartner,
    );
    ...
    try {
      const courier = this.courierFactory.getCourier(dto.courierPartner);
      ...
      await this.orderRepository.updateStatus(order.id, OrderStatus.CREATED);
    } catch (error) {
